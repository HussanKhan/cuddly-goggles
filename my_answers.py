...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # as shown below.
        self.activation_function = lambda x : sigmoid  # Replace 0 with your sigmoid calculation.
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            layer_2_output, layer_1_output = self.forward_pass_train(X)  # Implement the forward pass function below
            
            # Implement the backproagation function below
            delta_weights_i_h, delta_weights_h_o = self.backpropagation(layer_2_output, layer_1_output, X, y, 
    ...
    def forward_pass_train(self, X):
        ...
        layer_1_output = self.activation_function(X.dot(self.weights_input_to_hidden)) # signals from hidden layer
        
        # TODO: Output layer - Replace these values with the appropriate calculations.
        layer_2_output = layer_1_output.dot(self.weights_hidden_to_output) # signals into final output layer
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        # spread error properly
        hidden_error = self.weights_hidden_to_output.dot(error)
        print("wo ", self.weights_hidden_to_output.shape)
        ...
        delta_weights_h_o += error * np.array([hidden_outputs]).T
        delta_weights_i_h += hidden_error_term * np.array([X]).T      
        print("x ", X.shape)
        print("delta wo ", delta_weights_h_o.shape)
        print("delta ih ", delta_weights_i_h.shape)
        exit()
